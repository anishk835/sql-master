...
public class MergeTableImpl implements MergeTables {
	...
	@Override
	public SQLTable mergeTable(SQLTable... tables) {
		SQLTable finalTable = new SQLTable();
		...
		finalTable.setTableNames(tables[0].getTableNames() + "_" + tables[1].getTableNames());
		return finalTable;
	}

	private void mergeTwoTable(SQLTable finalTable, SQLTable[] tables) {
		List<String> userCol = tables[0].getColumns();
		List<String> purchaseCol = tables[1].getColumns();
